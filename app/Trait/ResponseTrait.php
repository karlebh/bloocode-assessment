...
trait ResponseTrait
{
    ...
    public function serverErrorResponse($message, ?\Throwable $exception = null): JsonResponse
    {
        if ($exception !== null) {
            Log::error(
    ...
    public function errorResponse(string $message = 'An error occurred', int $code = 500, array $data = [], ?\Throwable $exception = null)
    {
        if ($exception != null) {
            Log::error(
                "{$exception->getMessage()} on line {$exception->getLine()} in {$exception->getFile()}"
    ...
    public function badRequestResponse(string $message = '', int $code = 400, array $data = [], ?\Throwable $exception = null)
    {
        if ($exception != null) {
            Log::error(
                "{$exception->getMessage()} on line {$exception->getLine()} in {$exception->getFile()}"
